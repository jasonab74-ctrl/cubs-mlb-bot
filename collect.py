...
def clean(s:str)->str:
    s=_html.unescape(s or ""); s=re.sub(r"<.*?>","",s); return s.replace("\xa0"," ").strip()
...
def fmt_clock(dt): h=dt.strftime("%I").lstrip("0") or "0"; return f"{h}:{dt.strftime('%M')} {dt.strftime('%p')}"
def nice_when(ts:int, raw:str)->str:
    try: dt = datetime.fromtimestamp(ts, tz=timezone.utc) if ts>0 else None
    except: dt=None
...
POS_BASEBALL = [
    ...
    "call-up","optioned","waivers","trade","minor league","prospect","double-a","triple-a","aaa","aa",
]
NEG_OTHER = [
...
def is_cubs_baseball(t:str,s:str,l:str)->bool:
    txt=f"{t} {s} {l}".lower()
    if any(n in txt for n in NEG_OTHER): return False
    if any(p in txt for p in POS_STRONG): return True
    if "cubs" in txt or "wrigley" in txt or "chicago" in txt:
        if any(b in txt for b in POS_BASEBALL): return True
    if ("mlb" in txt or "baseball" in txt) and "cubs" in txt: return True
    return False
...
def main():
    ...
    payload={"updated":datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S %Z"),"items":items}
    with open(OUTFILE,"w",encoding="utf-8") as f: json.dump(payload,f,ensure_ascii=False,indent=2)
